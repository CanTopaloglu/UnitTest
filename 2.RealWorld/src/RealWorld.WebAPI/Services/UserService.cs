    ...
    IUserRepository userRepository, ILoggerAdapter<UserService> logger) : IUserService
{
    public async Task<List<User>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogError(ex, "User listesini geçerken bir hata ile karşılaştık");
            throw;
    ...
    public async Task<bool> DeleteByIdAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        {
            stopWatch.Stop();
            logger.LogInformation("Kullanıcı id'si {0} olan kullanıcı kaydı {1} ms de silindi", user.Id,
                stopWatch.ElapsedMilliseconds);
        }
    }
    ...
    public async Task<bool> UpdateAsync(UpdateUserDto request, CancellationToken cancellationToken = default)
    {
        ...
        logger.LogInformation("{0} kullanıcının güncelleme işlemi yapılmaya başlandı", request.Name);
        var stopWatch = Stopwatch.StartNew();
        ...
        {
            logger.LogError(ex, "Kullanıcı güncelleme esnasında bir hatayla karşılaştı");
            throw;
        }
        finally
        {
